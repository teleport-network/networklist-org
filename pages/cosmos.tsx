...
import Chain from '../components/cosmos-chain/chain'
...
import classes from './cosmos.module.css'
...
function Home({ changeTheme, theme }) {
  const { data, error } = useSWR<CosmosChainInfomation[]>('https://teleport-network.github.io/chains/cosmos_chains.json', fetcher)
              ...
              {/* { hideAd === '0' && <MultiChain closeMultichain={ closeMultichain } /> } */}
              {
                data && data.filter((chain) => {
                  if(search === '') {
                    ...
                    //filter
                    return chain.chainId.toString().toLowerCase().includes(search.toLowerCase()) ||
                    chain.rpc.toLowerCase().includes(search.toLowerCase()) ||
                    (chain.currencies[0] ? chain.currencies[0].coinDenom : '').toLowerCase().includes(search.toLowerCase())
                  }
                }).map((chain, idx) => {
